...
from torch.nn import Module, GRUCell, Linear, ModuleList, Dropout
...
class AttentionDecoder(Module):
    ...
    def __init__(self,
        ...
        self.attention: Linear = Linear(
            in_features=self.input_dim + self.output_dim,
            out_features=1,
            bias=False)
        ...
        self.extra_rnn_layers: ModuleList = ModuleList()
        for i in range(nb_gru_layers):
            self.extra_rnn_layers.append(
                GRUCell(self.output_dim, self.output_dim))
